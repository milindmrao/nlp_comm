...
class LSTMCellDecoder(tf.contrib.rnn.LSTMCell):
    ...
    embedding=None):
        ...
        super(LSTMCellDecoder, self).__init__(num_units,input_size,use_peepholes,
             cell_clip,initializer,num_proj,proj_clip,num_unit_shards,num_proj_shards,
...
def test_decoder():
    ...
    configs.numb_layers_enc = 3  # number of biLSTM layers at the encoder
    configs.numb_tx_bits = 30  # number of transmission bits
    configs.LSTMsizes = [9]  # The size of each layer of the LSTM decoder
    ...
    configs.numb_layers_dec = 1  # number of LSTM layers at the decoder
...
#            cell = tf.contrib.rnn.LSTMCell(num_units=self.LSTM_size[0], state_is_tuple=True)
            cell = LSTMCellDecoder(num_units=self.LSTM_size[0], state_is_tuple=True,feature_size=self.feature_size,embedding=embeddings)
            init_state = tf.contrib.rnn.LSTMStateTuple(*tf.split(denseOut,num_or_size_splits=2,axis=1))
            print(cell)
        if self.numb_layers_dec>1:
            cell = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.LSTMCell(num_units=cell_size, state_is_tuple=True)
