...
class EncChanDecNN(object):
    def __init__(self, config):
        ...
        self.enc_input = tf.placeholder(
            shape=[None, None], dtype=tf.int32, name='enc_input')
        self.enc_input_len = tf.placeholder(
            shape=[None], dtype=tf.int32, name='enc_input_len')
        self.dec_input = tf.placeholder(
            shape=[None, None], dtype=tf.int32, name='dec_input')
        self.dec_input_len = tf.placeholder(
            shape=[None], dtype=tf.int32, name='dec_input_len')
        self.target_sentense = tf.placeholder(
            shape=[None, None], dtype=tf.int32, name='target_sentense')
        # self.target_sen_len = tf.placeholder(
        #     shape=[None], dtype=tf.int32, name='target_sen_len')
        ...
        with tf.variable_scope(
            ...
            self.loss = self.setup_mask_loss()
    ...
    def setup_system(self):
        ...
        # encoder = Encoder(self.sentence_embeds, self.enc_input_len, self.config)
        encoder = EncoderStackedBiLSTM(self.embeddings, self.enc_input, self.enc_input_len, self.config)
        ...
        decoder = DecoderMultiLSTM(channel.chan_output, self.embeddings, self.dec_input, self.dec_input_len,
        ...
        with tf.name_scope("CrossEntLoss"):
            loss = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(
                    logits=self.decoder.dec_output, labels=self.target_sentense))  # Need to add masks
            tf.summary.scalar("CrossEntLoss", loss)

        return loss

    def setup_mask_loss(self):
        """
        Loss function with masking.
        """
        with tf.name_scope("CrossEntLoss"):
            cross_ent = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.decoder.dec_output,
                                                                       labels=self.target_sentense)
            mask = tf.sequence_mask(self.enc_input_len, self.batch_max_len)
            loss = tf.reduce_mean(tf.nn.weighted_moments(cross_ent, -1, mask))
            tf.summary.scalar("CrossEntLoss", loss)
        return loss
    ...
    def _create_feed_dict(self, batch, batch_len, lr):
        enc_input, enc_len, dec_input, dec_len, target = batch
        feed_dict = {}
        feed_dict[self.enc_input] = enc_input
        feed_dict[self.enc_input_len] = enc_len
        feed_dict[self.dec_input] = dec_input
        feed_dict[self.dec_input_len] = dec_len
        feed_dict[self.target_sentense] = target
    ...
    def _train_on_batch(self, session, batch, batch_len, lr=0.1):

        input_feed = self._create_feed_dict(batch, batch_len, lr)
        _, loss, tb_summary = self.optimize(session, input_feed)
        return (loss, tb_summary)

    def _run_epoch(self, session, train_data, tb_writer):
        batches, batch_lens = batches_pads(train_data, self.config.batch_size,
                                           self.config.batch_reader_param)
        tic = time.time()
        for i, batch in enumerate(batches):
            lr = 0.001 / np.sqrt(self.tb_itr_counter)
            loss, tb_summary = self._train_on_batch(session, batch, batch_lens[i], lr)
    ...
    def decode(self, session, sentence):
        ...
        sentence_batch = [list(
            map(lambda x: self.w2n.get(x.lower(), 0), word_token.tokenize(sentence)))] * self.config.batch_size
...
def dataset_to_token(file_path, word2num):
    ...
    for single_sentence in sentence_raw:
        ...
        if sum([x == 0 for x in tokens]) / len(tokens) < 0.2:
            tokens = [UNK_ID if x == 0 else x for x in tokens]
            sentences += [tokens]
...
def batches_pads(sentences_inp, batch_size=32, params={}):
    ...
    Returns:
        list of [(sentence_batch_inp,sentence_len_batch_inp,sentence_batch_out,sentence_len_batch_out)],len_batches
    Note:
        <pad> is PAD_ID
    """
    padded_batches = []
    ...
    if params.get('bucket', False):
        ...
        for ind in range(0, int(len(sentences) / batch_size) * batch_size, batch_size):
            ...
            sen_len_batch_inp = [len(x) + 1 for x in batch]  # add one for end of sentence indicator
            max_len_batch = max(sen_len_batch_inp)
            batch_enc_inp = [x + [END_ID] + [PAD_ID] * (max_len_batch - len(x) - 1) for x in batch]
            batch_dec_inp = [[START_ID] + x + [END_ID] + [PAD_ID] * (max_len_batch - len(x) - 1 + extra_out_pad) for x
                             in
                             batch]
            batch_targ_inp = [x + [END_ID] + [PAD_ID] * (max_len_batch - len(x) - 1 + extra_out_pad) for x in
                              batch]
            sen_len_batch_out = [x + extra_out_pad for x in sen_len_batch_inp]
            padded_batches += [(batch_enc_inp, sen_len_batch_inp, batch_dec_inp, sen_len_batch_out, batch_targ_inp)]
            len_batches += [max_len_batch + extra_out_pad]
...
if __name__ == '__main__':
    ...
    train_data_path = os.path.join(parent_dir, 'data', 'training_euro.pickle')
    valid_data_path = os.path.join(parent_dir, 'data', 'training_rest_reuters.pickle')
    ...
    # print("DataLoaded")
    # batches, max_len_batch = batches_pads(train_data, 2, config.batch_reader_param)
    # encInput, encLen, decInput, decLen, targ = batches[0]
    # print(train_data[0])
    # print(encInput[0])
    # print(decInput[0])
    # print(targ[0])
